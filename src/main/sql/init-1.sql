...
CREATE PROCEDURE zapsat_do_deniku(IN p_id INT UNSIGNED)
	BEGIN
		...
		SELECT
			...
			uv.cislo_vypisu,
			uv.banka
		INTO v_datum, v_porad_cislo, v_typ_platby, v_castka, v_poplatek, v_vs, v_user_id, v_nazev_protiuctu, v_cislo_protiuctu, v_poznamka, v_zprava, v_cislo_vypisu, v_banka
		FROM parsovane_vypisy pv
			JOIN uploadovane_vypisy uv ON (pv.ID_VYPISU = uv.id)
		WHERE pv.id = p_id;
		...
		ELSEIF v_banka = 'Raiffeisenbank a.s.'
			THEN
